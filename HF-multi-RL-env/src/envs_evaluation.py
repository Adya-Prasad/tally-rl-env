r"""
Compare both environments across the paper's components,
...
Key realization this phase delivers:
  ...
  Math is that loop with exactly ONE iteration. Wordle is the loop with up to six. They are not two different things. 
"""
...
# into one common record so a single "collector" can read either. 
...
# NOTE: it doesn't know or care that one is single-turn and one is multi-turn. 
